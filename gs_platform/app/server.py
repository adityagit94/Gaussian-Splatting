...
# Per-job locks to prevent concurrent truncation/writes
...
def save_job(job: Job) -> None:
    """
    Atomic save: write tmp then os.replace() to avoid partial/empty JSON reads.
    Also protected by per-job lock to avoid concurrent writes.
    """
    lock = get_job_lock(job.id)
    with lock:
        p = job_meta_path(job.id)
        tmp = p.with_suffix(".json.tmp")
        data = json.dumps(asdict(job), indent=2)
        tmp.write_text(data, encoding="utf-8")
        os.replace(tmp, p)

def load_job(job_id: str) -> Job:
    """
    Read job.json. If it is temporarily empty/partial (race),
    ...
    raise HTTPException(503, f"Job metadata temporarily unreadable (corrupt/empty): {last_err}")
...
def list_jobs() -> List[Job]:
    ...
    for d in sorted(JOBS_DIR.iterdir(), key=lambda p: p.name, reverse=True):
        ...
        try:
            ...
            # skip corrupted entries; they can be deleted manually
            continue
    return jobs
...
def home(request: Request):
    return templates.TemplateResponse(
        "index.html",
        {"request": request, "video_exts": ", ".join(sorted(VIDEO_EXTS))},
    )
...
def job_page(request: Request, job_id: str):
    ...
    return templates.TemplateResponse(
        "job.html",
        {"request": request, "job_id": job_id, "job": job, "status": job, "meta": meta},
    )
...
def browse_mkdir(
    ...
    curp = base
    parent = str(curp.parent) if curp.parent != curp else str(curp)
    video_only = (target == "video" and mode == "file")
    dirs, files, _ = list_dir(curp, mode, q, video_only)
